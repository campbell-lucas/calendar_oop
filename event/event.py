...
class Event:
    ...
    def duration(self, value):
        ...
        if not (10 < value < 600):
            raise ValueError(f'Too short or too long duration: {value} minutes')
    ...
    def start_time(self, value):
        if not isinstance(value, str):
            raise TypeError(f'Invalid value, please do not use type string: {value}')

        try:
            parsed_date = datetime.strptime(value, '%d-%m-%Y %H:%M')
        except ValueError:
            raise ValueError(f'Invalid date format, use DD-MM-YYYY HH:MM, {value}')
